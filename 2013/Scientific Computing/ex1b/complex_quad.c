...
int complex_quad(double complex a2, double complex a1, double complex a0, double complex* r1, double complex* r2){
	...
	a_range = max(max(cabs(a2),cabs(a1)),max(cabs(a1),cabs(a0))) - min(min(cabs(a2),cabs(a1)),min(cabs(a1),cabs(a0))); // dividing through reduces risk of fp overflow
	// DEBUG PRINTS
	printf("max = %.5g\nmin = %.5g\n", max(max(cabs(a2),cabs(a1)),max(cabs(a1),cabs(a0))), min(min(cabs(a2),cabs(a1)),min(cabs(a1),cabs(a0))));
	if (cabs(a_range) < DBL_MAX && cabs(a_range) > 1.0e+100){	
